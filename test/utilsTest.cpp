...
class IsValidTest : public ::testing::Test
{
  protected:
    ...
    // 辅助函数：创建测试目录
    fs::path
    createTestDirectory (const std::string &dirname)
    {
...
TEST_F (IsValidTest, NonExistentFileThrowsFileNotFoundException)
{
    ...
    EXPECT_THROW (
        { isValide (non_existent_file); }, ExcelReader::FileNotFoundException);
}
...
TEST_F (IsValidTest, DirectoryPathThrowsPathNotFileException)
{
    ...
    EXPECT_THROW (
        { isValide (directory_path); }, ExcelReader::PathNotFileException);
}
...
TEST_F (IsValidTest, EmptyFileNameThrowsException)
{
    ...
    EXPECT_THROW (
        { isValide (empty_path); }, ExcelReader::FileNotFoundException);
}
